...
p<-ggplot()+geom_sf(data=x)+geom_sf(data=y)
p
```
...
```{r}
y<-vector(mode='list')
y$read_me<-(" this is a list created from first script in ch2 seq , 'ch_2_tc_argo_matchup_eastern_pacific.Rmd'created 2023-09-25. $function_variablebles give parameters for selected objects. track issue should be fixed")
