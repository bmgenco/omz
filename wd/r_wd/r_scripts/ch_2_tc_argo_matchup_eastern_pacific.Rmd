...
### slect floats witthin desired radi
...
# h.multipoints<-as_data_frame(unique(h.pts$Key)) #depreciated
h.multipoints<-as.data.frame(unique(h.pts$Key))

names(h.multipoints)<-("Key")
st_geometry(h.multipoints)<-(split(h.pts, h.pts$Key) %>% Map(st_combine,. )%>%do.call(c,.))

h.multipoints<-dplyr::left_join(x, h.multipoints) %>%st_as_sf(.)
...
## match flots to tracks temporally and add distance
...
this does not exclude by strom strength
...
# 
# 
# f.storm_strength_filter<-function(b, h.storm_list, storm_level=NULL){
# # strom_level argument should  be one of ("all", "tc", "hu")
#     
...
#     }
# 
# f.selection<-function(x, tc_selection){x<-filter(x, Status %in% tc_selection)
# return(x)}
# h.storm_list<-lapply(h.storm_list, f.selection, tc_selection=tc_selection)
# h.storm_list<-f.rm_empty(h.storm_list) %>%names(.)
# 
# 
# b<-b[intersect(names(b), h.storm_list)]
# return(b)
# }
# 
# # need to make a list and function
# 
# tc_100<-lapply(km_100, f.storm_strength_filter, h.storm_list, storm_level="tc")
# hu_100<-lapply(km_100, f.storm_strength_filter, h.storm_list, storm_level="hu")
# 
# tc_200<-lapply(km_200, f.storm_strength_filter, h.storm_list, storm_level="tc")
# hu_200<-lapply(km_200, f.storm_strength_filter, h.storm_list, storm_level="hu")
# 
# tc_500<-lapply(km_500, f.storm_strength_filter, h.storm_list, storm_level="tc")
# hu_500<-lapply(km_500, f.storm_strength_filter, h.storm_list, storm_level="hu")
# 
# 
# length(tc_100)
# length(hu_100)
...
storm_list<-names(all_storms_500km)
h.storm_list<-filter(h, Key %in% storm_list) %>% dplyr::select(., Key, Status)%>% split(., .$Key) 
# f.unique<-function(x){x<-unique(x$Status)}
# test<-lapply(h.storm_list, f.unique)


#v2a
f.storm_strength_filter<-function(h.storm_list, storm_level=NULL){
...
h.hu<-f.storm_strength_filter(h.storm_list, storm_level = 'hu')
h.tcs<-f.storm_strength_filter(h.storm_list, storm_level = 'tc')
h.all<-f.storm_strength_filter(h.storm_list, storm_level = 'all')

f.intersect<-function(x, selection) {
  x<-x[intersect(names(x), selection)]
  return(x)
}
tc_100<-f.intersect(km_100, selection=h.tcs)
hu_100<-f.intersect(km_100, selection=h.hu)
...
# # TC strength stats
# f.tc_stats<-function(x, h.tracks)
# x.key<-names(x)
# tcs<-filter(h, Key %in% x.key )%>% select(., Key, Status)
