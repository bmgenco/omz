...
# subset h by ocena basins
 * this a brute force method to rmeove a few tracks form a strom(s) that passed from Altantic over central ameriaca in to the pacific
...
```{r}
df.profile_points <- ldply(profile_points, data.frame)
...
# lon_lat<-do.call(rbind, st_geometry(h.pts))%>%as.data.frame(.) %>% setNames(c("Lon","Lat"))
# h_n<-cbind((st_drop_geometry(h.pts)), lon_lat)


x<-h.pts%>% st_drop_geometry(.) %>% select(., Key, DateTime) %>% group_by(Key) %>% summarize(min_dt= min(DateTime),max_dt = max(DateTime))

h.tracks<-h.pts %>% select(Key, geometry) %>% group_by(Key) %>% st_cast("LINESTRING")
h.tracks<-left_join(x, h.tracks)
rm(x)
...
x$min_dt<-x$min_dt-(pre)

f.in_between<-function(h.tracks){
 y<-(x[1,2]) %>% as.Date(.)
  
  
  x$min_dt<-x$min_dt-days(pre)
