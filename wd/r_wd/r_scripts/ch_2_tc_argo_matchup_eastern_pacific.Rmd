...
# lapply function: 
  ...
  y$DateTime<-
  f.time_estimation<-function(y,pt){
    i<-1 #testing
    pt<-z[i,] #tesing
    ...
    #ad if else staement here
    if(np =end){pt$overpass_time =np$DateTime} else if (np = first) {pt$overpass_time =np$DateTime} else {
    ...
    if(fld < pld){
      ...
      direction <-"p"}
    f.overpass_time<-function(np,fp, pp, distance) {}
    
    pt$overpass_time<-f.overpas
    }
  ...
  q<-vector(mode='list', length(z))
  for(i in 1:length(q)){
   i<-1 #tesing 
   pt<-z[i,]
   pt<-f.time_estimation(y,pt)
   
   
  q<-as.data.frame(do.call(rbind, q))
  return(w) 
  

q<-lapply(q, f.per_storm_t, h.pts=h.pts)
