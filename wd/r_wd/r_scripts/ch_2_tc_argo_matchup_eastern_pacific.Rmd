...
i<-st_nearest_points(p,l) %>% st_cast(., "POINT") %>% st_as_sf(.)
st_geometry(i) <- "geometry"
b<-rbind(i,p)

test<-b[!(duplicated(b$geometry) | duplicated(b$geometry, fromLast = TRUE)), ]


np<-st_intersects(i,p)
test<-st_join(i,p, left=T)
