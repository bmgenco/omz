...
## convert huricane poiutns to multi points
...
## get point of intersection of profile to h.track line
...
# w<-vector(mode='list', length = length(z)) this was in error in past
...
## aproximate tiem of over pass, nearest  6 hour point is time.. sigh not goode enought for global analysis
...
## some plots
* save iamge for manuscript
  -- crop reduce size and 
...
### add points and time of overlap
points
error her in olaf?
```{r}
# points
# w<-f.point_of_overpass(z=all_storms_500km,h.tracks)
all_storms_500km<-f.point_of_overpass(z=all_storms_500km,h.tracks)
all_storms_500km<-lapply(all_storms_500km, na.omit) # for some reason f.point of overpass creates empty rows which brak next steps
# time relative


```

### add time of over pas in and rleativ edays
```{r}

all_storms_500km<-f.time_relative_overpass(z=all_storms_500km, h.pts=h.pts)

```


# get stats on tcs with before and after

```{r}
test<-all_storms_500km$CP022006

# orignal v1 
# f.signs<-function(x){
#   x$s<-mean(sign(x$day_difference))
# return(x)}
# z<-lapply(all_storms_500km, f.signs)
# f.filter<-function(x){
#   x<-filter(x, x$s == 0)
#   return(x)
# }
...
# v2
f.signs_1<-function(x){
  x$s<-sign(x$day_difference)
  # counting zero as after
  x$s[x$s ==0]<-1
  x$s<-as.factor(x$s)
  if(length(levels(x$s))==2){x<-x} else {x<-NULL}
  
return(x)}
z<-lapply(all_storms_500km, f.signs_1)
z<-z[lengths(z) !=0]

f.signs_2<-function(x){
  x$s<-sign(x$day_difference)
  # counting zero as after
  x$s[x$s ==0]<--1
  x$s<-as.factor(x$s)
...
```{r}
# setwd(wd)
# saveRDS(all_storms_500km, "r_objects/all_storms_500km.R")
# saveRDS(h.pts, "r_objects/h.pts.R")
# all_storms_500km<-readRDS("r_objects/all_storms_500km.R")
# h.pts<-readRDS("r_objects/h.pts.R")
```


#####  not fucntioninfg fucntion 
time aproximate  just gets the time of neae h.pts. other function cant get to work... sigh
```{r}
##### function v2 ####
...
# z<-Map(cbind, z, Key= names(z))
# z<-z[[8]]
# t1<-ggplot()+geom_sf(data=st_as_sf(z))
# t2<-t1+geom_sf(data=st_as_sf(z$overpass_point), col= "blue")
# 
# tmp.key<-unique(z$Key)
# track<-filter(h.tracks, Key==tmp.key)
# t3<-t2+geom_sf(data=track)
# t3
...
# z<-all_storms_500km # for use if not using sign elmination
...
```{r}
y<-vector(mode='list')
y$read_me<-(" this is a list created from first script in ch2 seq , 'ch_2_tc_argo_matchup_eastern_pacific.Rmd'created 2023-10-04 -16:12. $function_variablebles give parameters for selected objects. Fixed error in slecton function that incoreclte filter floats and profiel per storm. improved  before/after slection. now removed stroms without bot before and after. use this version fior suplemnatl figures")
