...
```{r}
setwd(wd)
write.csv(times, "station3_local.csv")
...
# station 3
```{r}
plot(d3, which="oxygen2", ztype="image", xtype="time")
plot(d3, which=3, ztype="image", xtype="time", ylim=c(-20,80), add=T) # density
plot(d3, which=c("oxygen2", "density"), ztype="point", xtype="time", ylim=c(-20,70)) # density

plot(d3, which=c("oxygen2", "beamAttenuation", "density"), ztype="image", xtype="time", ylim=c(-20,70)) # density
plot(d3, which=c("oxygen2", "beamAttenuation", "density"), ztype="image", xtype="time")

plot(d3, which="fluorescence", ztype="image", xtype="time", ylim=c(-20,80))
plot(d3, which="par", ztype="image", xtype="time", ylim=c(-20,70)) #not neede
plot(d3, which="par", ztype="points", xtype="time", ylim=c(-20,70)) #not needed


```
## station 3 oceanogrpahic plots
...
```{r}
plot(d3, drawIsobaths=T, which="map")
plot(d3, which=3, ztype="image")
# plot(d35, which="salinity", ztype="image", xtype="time")
# plot(d35, which="temperature", ztype="image", xtype="time")
plot(d3, which="oxygen2", ztype="image", xtype="time")
plot(d3, which="fluorescence", ztype="image", xtype="time")


plot(d3, which="oxygen2", ztype="image", xtype="time", ylim=c(-20,180))

plot(d3, which=3, ztype="image", xtype="time", ylim=c(-20,180)) # density

plot(d3, which="fluorescence", ztype="image", xtype="time", ylim=c(-20,180))
# plot(d3, which="par", ztype="image", xtype="time", ylim=c(-20,180)) #not needed
...
plot(c5, which=c("oxygen2", "density", "temperature", "beamAttenuation", "fluorescence"), type="l", plim=c(180, 0))
...
par(mfrow = c(2, 2))
plotProfile(c5, xtype="density+N2", ylim=c(180,10),  ytype="depth")
plotProfile(c2, xtype="salinity+temperature", ylim=c(180,10))
 plotProfile(c1, xtype="spice", ylim=c(180,10),  ytype="depth")
plotProfile(c1, xtype="Rrho", ylim=c(180,10),  ytype="depth")
...
Read me
https://stackoverflow.com/questions/52703938/decomposing-time-series-data-no-or-less-than-2-periods
...
ts.sst<-ts(x1$avg_SST_C, frequency=(365/8), start=c(2002, 07))
ts.chl<-ts(x1$avg_CHL_mg_m3_day, frequency=52, start=c(2002, 07))


d.noise_chl<-decompose(noise.chl, type = "multiplicative")
d.noise


d.sst<-decompose(ts.sst, type = "multiplicative")
d.chl<-decompose(ts.chl, type = "multiplicative")
