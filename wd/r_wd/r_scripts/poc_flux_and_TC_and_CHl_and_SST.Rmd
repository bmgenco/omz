...
# 
# x<-0.1
# scale.pres<-scale.pres+x

# scale.pres<-scale.pres*5


capt<-'8-Day resolution: Non-lagged EF. Colored lines are individual tropical disturbances that pass within the export flux box around station 3.5: Orange = Hurricane strength,  Blue = less than " " '
...
# 
# Plot_2<-ggplot()+
# geom_line(aes(y=chl * scale.chl, x=date), color=chcolor, data=y)+
# geom_line(aes(y=sst * scale.sst, x=date), color=tcolor, data=z)+
...
# plot_2<- ggplot()+
# # geom_line(data=x, aes(date, avg), color=line_col, lwd=1)+
# # geom_point(data=h_rect, aes(start, y2*scale.pres), color=rect_fill)+
# # geom_point(data=t_rect, aes(start, y2*scale.pres), color=rect_fill_non)+ 
# # 
# # scale_y_continuous(name ="Mean export flux (mgC/m^2/day)", labels = function(x) format(x, scientific = TRUE), sec.axis = sec_axis(~./scale.pres,name="eye atmospheric pressure"))+
# # 
...
# geom_line(data=x, aes(date, avg), color=line_col, lwd=1)+
# geom_rect(data=t_rect, mapping=aes(xmin=start, xmax=end, ymin=-Inf, ymax=y2*scale.pres), color=rect_fill_non, fill=rect_fill_non, alpha=0.5)+
# geom_rect(data=h_rect, mapping=aes(xmin=start, xmax=end, ymin=-Inf*scale.pres, ymax=y2*scale.pres), color=rect_fill, fill=rect_fill, alpha=0.5)+
# 
# scale_y_continuous(name ="Mean export flux (mgC/m^2/day)", labels = function(x) format(x, scientific = TRUE), sec.axis = sec_axis(~./scale.pres, scale_y_reverse(),name="eye atmospheric pressure"), guide = guide_axis(order = 4))+
# 
# labs(title=lab_avg, caption = '8-Day resolution: Non-lagged EF. Colored lines are cyclones that pass within export flux box around station 3.5: Orange = Hurricane strength,  Blue = less than " " ')+
# theme(plot.title = element_text(hjust = 0.5))+
# theme(plot.caption =element_text(hjust = 0.5))+
