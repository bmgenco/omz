...
# save all
# station_circle_list<-list(circle_1, circle_2, cir  cle_3, circle_3.5, circle_4, circle_5 )
...
##### count ing hurricanes
-just for inserting statitics into paper
```{r}

# temp<-filter(h_4, Status =="HU")
# length(unique(temp$Key))
...
q2<-q %>%group_by(year) %>% arrange(DateTime) %>% mutate(days_interval = difftime(DateTime, lag(DateTime), unit="days"))
# q2<-na.omit(q2)


q2$days_interval=as.numeric(q2$days_interval)
 q2<-filter(q2, days_interval != 0.25000000)
 q2<-filter(q2, days_interval >=1)
 
 
 q2<-select(q2, Key, Name, DateTime, days_interval, Status, year)


which(q2$days_interval ==0.04166667)
# q$md<-format(as.Date(q$DateTime), "%m-%d")
q1<-q %>%group_by(year) %>% summarise(., m=mean(DateTime), f=min(DateTime), l=max(DateTime), md=mean(md), 
middle=median(DateTime)) 
q1$fyd<-yday(q1$f)
q1$lyd<-yday(q1$l)
q1$window<-q1$lyd-q1$fyd
mean(yday(q1$middle))
mean(q1$fyd)



q2<-q %>%group_by(year) %>% summarise(count=n_distinct(Key))
```
...
title<-paste0("Station ", "3.5", ": ", "Average Export Flux and storms within 400km^2 box")
...
# 
all_400_removed<-f.remove_close_storms(x=all_400, pre=60)

# subtract names from to get eculsion list
