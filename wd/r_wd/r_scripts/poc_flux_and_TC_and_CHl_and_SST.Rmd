...
breaks<-c( 872, 900, 920, plyr::round_any((944+920)/2, 5, f=floor), plyr::round_any((964+945)/2, 5, f=ceiling), plyr::round_any((979+965)/2,5, f=floor), 980, 1000) %>% round(., 0) %>% append(., c(round(stdp,2),  round(adj,2)))

labs<-(breaks)
breaks<-abs(breaks-adj)
...
geom_rect_pattern(data=t_4_rect, mapping=aes(xmin=start, xmax=end, ymin=-Inf, ymax=y2*scale.e), color=rect_fill_outside, fill=rect_fill_outside, pattern = 'crosshatch', pattern_fill= "grey30", alpha=0.5)+

geom_rect(data=t_rect, mapping=aes(xmin=start, xmax=end, ymin=-Inf, ymax=y2*scale.e), color=rect_fill_non, fill=rect_fill_non, alpha=1)+
geom_rect(data=h_rect, mapping=aes(xmin=start, xmax=end, ymin=-Inf, ymax=y2*scale.e), color=rect_fill, fill=rect_fill, alpha=1)+  

geom_line(data=w, aes(date, chl), color=chcolor, lwd=0.2)+


geom_point(data=w, aes(date, chl), color=chcolor, lwd=.75)+ 
scale_y_continuous(name =txt$ylab, sec.axis = sec_axis(~./scale.e, name=txt$ylab2,  breaks = breaks , labels= labs))+
