...
# scale.sst<-max(x$avg) / max(z$sst)
# scale.chl<-max(x$avg) / max(y$chl)
...
p.breaks<-c( 872, 900, 920, plyr::round_any((944+920)/2, 5, f=floor), plyr::round_any((964+945)/2, 5, f=ceiling), plyr::round_any((979+965)/2,5, f=floor), 980, 1000) %>% round(., 0) %>% append(., c(round(stdp,2),  round(adj,2)))
# p.breaks<-c(872, 900, 920, 940, 960, 980, 1000, stdp, adj)

# labs<-p.breaks

labs<-(p.breaks)
p.breaks<-abs(p.breaks-adj)


scale.pres<-(max(x$avg)/max(c(t_rect$y2, h_rect$y2)))
...
geom_hline(yintercept=(mean(x$avg)+sd(x$avg)), data=x, linetype="dashed", color ="grey")+
geom_rect(data=t_rect, mapping=aes(xmin=start, xmax=end, ymin=-Inf, ymax=y2*scale.pres), color=rect_fill_non, fill=rect_fill_non, alpha=0.5)+
geom_rect(data=h_rect, mapping=aes(xmin=start, xmax=end, ymin=-Inf, ymax=y2*scale.pres), color=rect_fill, fill=rect_fill, alpha=0.5)+
# geom_line(data=x, aes(date, avg), color=line_col, lwd=1)+
geom_point(data=x, aes(date, avg), color=line_col, lwd=1)+
  
# scale_y_continuous(name ="Mean export flux (mgC/m^2/day)", labels = function(x) format(x, scientific = TRUE),
scale_y_continuous(name ="Mean export flux (mgC/m^2/day)", sec.axis = sec_axis(~./scale.pres,name="Cyclone eye pressure - millibars (mb)",  breaks = p.breaks , labels=labs), limits = c(poc_lim, NA))+
  
labs(title=lab_avg, caption = capt)+
...
#chl is in mg / m**3, so to convert o ug *100
...
geom_hline(yintercept=(mean(x$avg)+sd(x$avg)), data=x, linetype="dashed", color ="grey")+
geom_hline(yintercept=scale.chl*(mean(y$chl)+sd(y$chl)), data=y, linetype="dashed", color =fcolor)+
geom_line(data=x, aes(date, avg), color=line_col, lwd=.5)+
geom_line(data=y, aes(date, chl*scale.chl), color=fcolor, lwd=.5)+
scale_y_continuous(name ="Mean export flux (mgC/m^2/day)", sec.axis = sec_axis(~./scale.chl, name= "Chlorophyll-a [μg/L]"))+
labs(title=lab_avg, caption = capt)+
theme(plot.title = element_text(hjust = 0.5))+
theme(plot.caption =element_text(hjust = 0.5))+
scale_x_date(date_breaks = "1 years", date_labels = "%Y", date_minor_breaks = "6 months", position = "top")+
...
t.breaks<-seq(from=22, to =27, by =1)
labs<-t.breaks

adj<-floor(min(z$sst))
# labs<-plyr::round_any(t.breaks, 1, f=round)

t.breaks<-t.breaks-adj
z$sst<-z$sst-adj
...
geom_hline(yintercept=(mean(x$avg)+sd(x$avg)), data=x, linetype="dashed", color ="grey")+
geom_hline(yintercept=scale.sst*(mean(z$sst)-sd(z$sst)), data=z, linetype="dashed", color =tcolor)+
geom_line(data=x, aes(date, avg), color=line_col, lwd=.5)+
geom_line(data=z, aes(date, sst*scale.sst), color=tcolor, lwd=.5)+
scale_y_continuous(name ="Mean export flux (mgC/m^2/day)", sec.axis = sec_axis(~./scale.sst, name= "SST (°C)", breaks = t.breaks , labels=labs))+
labs(title=lab_avg, caption = capt)+
theme(plot.title = element_text(hjust = 0.5))+
theme(plot.caption =element_text(hjust = 0.5))+
scale_x_date(date_breaks = "1 years", date_labels = "%Y", date_minor_breaks = "6 months", position = "top")+
...
scale.chl_sst<-max(y$chl)/max(z$sst)

capt<-' MODIS derived Chl-a in green, and SST in red. Inputs of VGPM NPP model.'

plot_d<- ggplot()+ theme_bw()+
geom_hline(yintercept=(mean(y$chl)+sd(y$chl)), data=y, linetype="dashed", color =fcolor)+
geom_hline(yintercept=scale.chl_sst*(mean(z$sst)-sd(z$sst)), data=z, linetype="dashed", color =tcolor)+
geom_line(data=z, aes(date, sst*scale.chl_sst), color=tcolor, lwd=.5)+
geom_line(data=y, aes(date, chl), color=fcolor, lwd=.5)+
scale_y_continuous(name ="Chlorophyll-a [μg/L]", sec.axis = sec_axis(~./scale.chl_sst, name= "SST (°C)", breaks = t.breaks , labels=labs))+
labs(title=lab_avg, caption = capt)+
theme(plot.title = element_text(hjust = 0.5))+
theme(plot.caption =element_text(hjust = 0.5))+
scale_x_date(date_breaks = "1 years", date_labels = "%Y", date_minor_breaks = "6 months", position = "top")+
