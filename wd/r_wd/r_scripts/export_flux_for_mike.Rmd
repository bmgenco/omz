...
pdo<-download_pdo()
t.sst<-filter(t.sst, time<=cutoff)
pdo<-pdo%>% filter(Date>= min(t.sst$time), Date<=cutoff)
pdo$PDO<-scale(pdo$PDO)
t.pdo<-xts(pdo$PDO, pdo$Date)

# ccf need tim series to be smae smapling ferqucny..

t.chl$value<-scale(t.chl$value)
t.sst$value<-scale(t.sst$value)

t.pdo$value<-scale(t.pdo$value)
ts_plot(t.pdo,  t.sst, ts_trend(t.sst), -ts_trend(t.pdo))



ts_plot(-1*ts_trend(t.oni), -1*ts_trend(t.pdo))
