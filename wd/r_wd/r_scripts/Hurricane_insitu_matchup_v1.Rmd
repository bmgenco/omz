...
f.argo_by_database_w_clipFODZ<-function(database, maxf, year_begin, decimal){
  y<-filter(database, year(DateTime) >= year_begin) 
  z<-maxf %>% filter(., maxFODZ >= decimal)
  y<-st_difference(y,z)
  rm(z)
...
diva<-tidync(file)%>% activate(c("D0,D1")) %>% hyper_tibble(select_var =q)
rm(file, q)

# test<-tidync(file)
# maxf<-diva%>% select(., maxFODZ, Longitude, Latitude) %>%

maxf<-diva %>% sf::st_as_sf(., coords = c("Longitude","Latitude"), crs = "+proj=longlat +datum=WGS84 +ellps=WGS84")

maxf<- st_crop(maxf,xmin = function_variables$h.lon_min,ymin= function_variables$h.lat_min, xmax=function_variables$h.lon_max, ymax= function_variables$h.lat_max )



# m.map<-m.coast+geom_sf(data=test, aes(color=maxFODZ))+scale_color_cmocean(name="matter", direction =1)
...
match2<-f.argo_by_database_w_clipFODZ(database, maxf, year_begin = 2000, decimal=0.25)
...
```{r}
overpass_time<-match$`41272`$data$DateTime
dates<-Sprof$date[trial$`41272`]
