...
packages<-c("stars", "gridExtra", "cowplot", "sf", "ggspatial","stringr","sp", "rgdal",  "rgeos", "raster","readr" ,"tidyverse", "ggplot2", "lubridate",  "ggthemes", "data.table", "reshape2", "patchwork", "RColorBrewer", "marmap", "extrafont", "oce", "ncdf4", "raster", "RStoolbox", "cmocean", "metR")  # set packages here
f.ipak(packages)
...
# plot<-9
# test<-combined[[plot]]
# date<-paste0("June", " ", names(combined)[plot])
# 
# col<- colorRampPalette(c(
#   "#DBF7D8", "#B6DDAE", "#8FC788", "#64B365", "#21A049", "#078749", "#186D43", "#1E5338", "#1D3B29", "#152317"))
# 
# col<-colorRampPalette(c("#ffffff", "#DBF7D8", "#C9EAC3" ,"#B8DFB0" ,"#A6D49E", "#93CA8C" ,"#80BF7A", "#6BB66A" ,"#54AD5A", "#35A44C", "#0B9A49", 
#                         "#038D49", "#0B8148", "#147445","#196841", "#1C5D3D", "#1E5037", "#1E4530", "#1C3928", "#1A2E20", "#152317",
#                         "#423E85","#443882","#46327E", "#472C7A", "#482576","#481E6F", "#481769", "#471063", "#46085C","#440154"))   
# 
# lon<-as.numeric(rownames(test))
# lat<-as.numeric(colnames(test))
#   
# #cl<-st_crop(cl, xmin=min(lon), ymin=min(lat), xmax=max(lon), ymax=max(lat))
# #imagep(lon, lat, log10(test))
# #imagep(lon, lat, log10(test), col=col, zlim=log10(c(0.01, 20)), missingColor=1, zlab='log10(Chl)')
# 
# 
# #t1<-round(max(na.omit(as.vector(test))),1)
# 
# 
# # png(paste0(date, ".png"))
# 
# 
# imagep(lon, lat, (test), col=col, zlim=(c(0.01, 5)), missingColor=NULL, zlab='Chl (mg/m^3)')
# 
# 
# 
# 
...
# # setwd(fig)
# # png(paste0(date, ".png"))
# # imagep(lon, lat, (test), col=col, zlim=(c(0.01, 5)), missingColor=1, zlab='Chl (mg/m^3)')
# # plot(cl,  col='gray', add=T)
# # title(main=date)
# # dev.off()
# 
# 
...
# # six is the best
# 
# #cuts<-c(0.01, 0.04, 0.06, 0.08,  0.1, 0.15, 0.2, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2
# #cuts<-c(0.01, 0.04, 0.06, 0.08, 0.09, 0.1, 0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v1
# #cuts<-c(0.01, 0.04, 0.06, 0.08, 0.09, 0.1, 0.125, 0.15, 0.175, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v2
# #cuts<-c(0.01, 0.04, 0.06, 0.08, 0.09, 0.1,  0.15, 0.175, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v3
# #cuts<-c(0, 0.01, 0.04, 0.06, 0.08, 0.09, 0.1,  0.15, 0.175, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v4
# 
# #cuts<-c(0, 0.01, 0.04, 0.06, 0.08,  0.1,  0.15, 0.175, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v5
# cuts<-c(0, 0.01, 0.04,  0.08,  0.1,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v6
# #cuts<-c(0, 0.01, 0.04,  0.08, 0.09, 0.1,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v7
# #cuts<-c(0, 0.01, 0.05,  0.08, 0.09, 0.1, .11, .12,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v8
# #cuts<-c(0, 0.01, 0.05,  0.08, 0.09, 0.1, .125,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v9
# 
# 
# setwd(fig)
# pdf("figure_2_v6_small.pdf", height=2.85 , width =2)
# par(mfrow=c(4,3))
# 
# for(i in 1:length(combined)){
# i<-5
...
# 
# plotRGB(t2)
# plot(t3, col = rev(rainbow((length(cuts)+1))),  breaks=cuts, add=T, margin=F)
# 
# 
# 
# 
# #plot(t1, col = rev(rainbow((length(cuts)+1)),  zlim=(c(0.01, 5)), zlab='Chl (mg/m^3)'), breaks=cuts, add=T)
# }
# dev.off()
```

##### plotting attemp 3 nature geosciene ####
* legends per plot
```{r}
# options(warn=-1)
# 
# # cuts<-c(0, 0.01, 0.04,  0.08,  0.1,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)
# # cuts<-c(0, 0.01, 0.04,  0.08,  0.1,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v6
# #cuts<-c(0, 0.01, 0.04,  0.08, 0.09, 0.1,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v7
# #cuts<-c(0, 0.01, 0.05,  0.08, 0.09, 0.1, .11, .12,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v8
# #cuts<-c(0, 0.01, 0.05,  0.08, 0.09, 0.1, .125,  0.15, 0.2, 0.35, 0.5, 0.8, 0.9, 1, 2, 4, 6)# fig_2_v9
...
#         plot.margin = unit(c(t, 0, 0, 0.25), "cm"),
...
#   guides(fill=guide_colourbar(title=title_two, barheight = 8)) +
...
# 
# setwd(fig)
# pdf("figure_2.pdf", height=11.5 , width =8)
# plot_grid(plotlist=plots, ncol=3, nrow=4)
# dev.off()
# 
# ```
# 
# 
# #### plotting attempt 4 nature geosciene ###
# * see https://stackoverflow.com/questions/53918557/fix-legend-title-when-using-superscript-in-more-than-two-lines-r-ggplot2
# ** lines function might print better
# 
# ```{r}
# options(warn=-1)
# 
...
# cuts<-rescale(cuts)
# # title_two<-"Chl a\nmg/m^3"
# title_two<-expression(atop(textstyle(" Chl a "), textstyle(" mg⋅M"^-1*"")))
# #title_two<-list("Chl a", expression("mg⋅M"^-1*""))
# 
# labs<-c(0, .5, 1, 2 , 2.5, 5)
# 
...
#   return(f3)
# }
# f.fig2_legend<-function(combined, v_rgb, l, b, r){
#   t1<-combined[[1]]
#   t2<-v_rgb[[1]]
#   
#   lat<-as.numeric(row.names(t(t1)))
...
#     scale_fill_gradientn(colors = custom_color, values=cuts, na.value=NA,limits=c(0,5), 
#                          name=waiver(),breaks=labs)+
#     guides(fill=guide_colourbar(title=title_two, barheight = b)) +
#     theme(legend.title =element_text(size=8), legend.text=element_text(size=6)) + 
#     theme(text=element_text(family="Helvetica"))
#   
#   legend <- get_legend(p + theme(legend.box.margin = margin(0, 0, 0, l)))
#   rm(p, t3, t2) 
#   return(legend)
...
# legend<-f.fig2_legend(combined, v_rgb, l=1, b=150)
# 
# i<-1
# plots[[i]]<-f.fig2(combined, v_rgb, i=i, t=0, l=0)
# i<-2
# plots[[i]]<-f.fig2(combined, v_rgb, i=i, t=0, l=-2)
# i<-3
# plots[[i]]<-f.fig2(combined, v_rgb, i=i, t=0, l=-4)
# 
# 
# 
# 
# setwd(fig)
# pdf("testing3.pdf", height=11.5 , width =8)
# plot_grid(plotlist=plots, ncol=3, nrow=4)
# dev.off()
# 
# 
...
```
##### plotting attempt 5.A - function attempt
```{r}
# #plots<-vector(mode='list', length = length(combined))
# 
# t<-c(0,0,0,-3,-3,-3,-6,-6,-6, -9,-9,-9)
# t<-as.list(t)
# l<-c(0, -2.5, -5, 0, -2.5, -5, 0, -2.5,-5, 0, -2.5, -5)
# l<-as.list(l)
# 
# date<-paste0("June", " ", names(combined))
# 
# f.fig2<-function(x, y, z, w, date){
#   t1<-x
#   t2<-y
#   
#   
...
#   return(legend)
# } # l = legend margin left, b=barhieght (in cm ) of legend
# legend<-f.fig2_legend(combined, v_rgb, l=1, b=45, width)
# 
# plots<-mapply(f.fig2, x=combined, y=v_rgb, z=t, w=l, date=date, SIMPLIFY = F)
# 
