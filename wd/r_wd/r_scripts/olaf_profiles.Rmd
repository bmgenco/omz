...
packages<-c("sp", "rgdal",  "rgeos", "raster", "readr", "tidyverse", "lubridate",  "ggthemes",  "sf", "cmocean", "ncdf4", "RNetCDF",  "plot3D", "tidync", "devtools", "stars", "ncmeta", "maps", "oce","data.table","raster", "fasterize", "RStoolbox", "scales", "purrr", "marmap", "cowplot", "gsw", "R.utils","Matrix")
...
wd<-getwd()
```
## Load submodule for bio-argo floats older
...
setwd(data_d)
setwd("./ODV_exported")
# setwd("/home/brandon/vestawd/omz/data/2018_data/ODV_exported/")
woa<-read_tsv("isosurface_data.txt", skip=1) %>% dplyr::select(., 'Depth [m]  at  Oxygen [ml/l]=0.45 @ Depth [m]  at  Oxygen [ml/l]=0.45=first', 'Longitude', "Latitude")
...
# added 2022-07-27 simplifies stations
...
added 2023-10-09 3rd float


```{r}
initialize_argo()
new.float<-get_lon_lat_time(float_ids = as.numeric("5905068"))
# new.float<-load_float_data(float_ids=as.numeric("5905068"))
...
cols.tracks = c("6903093" = f1_color, "6903094" = f2_color, "5905068" =f3_color )
...
new.float<-as.data.frame(cbind(new.float$time[[1]], round(as.numeric(new.float$lon[[1]]),3), round(as.numeric(new.float$lat[[1]]),3)))
names(new.float)<-c("time", "lon", "lat")
new.float$lon<-as.numeric(new.float$lon)
new.float$lat<-as.numeric(new.float$lat)

f3<-sf::st_as_sf(new.float, coords=c("lon", "lat"), crs = "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")
track3<-st_geometry(f3) %>%  as(., 'Spatial')  %>% as(., "SpatialLines")%>% st_as_sf(.)
f3<-geom_sf(data=f3, color=f3_color, shape= argo_point_shape, size =dot_size)
...
t3<-geom_sf(data=track3, aes(fill = "5905068"), color=f3_color, linetype= "longdash", show.legend = "line", inherit.aes = F)
...
l3<-scale_fill_manual(name = "Argo Floats", values = cols.tracks, guide =guide_legend(override.aes = list(linetype=  c("dotted", "solid", "longdash"), color = cols.tracks)))
...
#floast not lines
m.map<-ggplot()+c1+c2+f1+f2+f3+p2a+p1b+ggtitle("Hurricane Olaf and Argo Float Tracks")+theme(plot.title = element_text(size = title.size))+s1+s3+s5+s7+s9+c3+l1+l2+th1+a1+y1

#lines

m.map1<-ggplot()+c1+c2+t1+t2+t3+l3+ggtitle("Hurricane Olaf and Argo Float Tracks")+theme(plot.title = element_text(size = title.size))+s1+s3+s5+s7+s9+c3+a1+y1+th2

t1<-geom_sf(data=track1,  color=f1_color, linetype= "dotted")
t2<-geom_sf(data=track2, color=f2_color, linetype= "solid")
t3<-geom_sf(data=track3, color=f3_color, linetype= "longdash")

m.map2<-ggplot()+c1+c2+t1+t2+t3+p2a+p1b+ggtitle("Hurricane Olaf and Argo Float Tracks")+theme(plot.title = element_text(size = title.size))+s1+s3+s5+s7+s9+c3+l1+l2+th1+a1+y1
m.map3<-ggplot()+c1+c2+t1+t2+t3+p2a+p1b+ggtitle("Hurricane Olaf and Argo Float Tracks")+theme(plot.title = element_text(size = title.size))+s1+s3+s5+s7+s9+c3+a1+y1+th2+guides(fill="none")
...
m.map4<-ggplot()+c1+c2+t1+t2+t3+p1a+p2a+p1b+ggtitle("Hurricanes Olaf & Bud with Argo Float Tracks")+theme(plot.title = element_text(size = title.size))+s1+s3+s5+s7+s9+c3+l1+l2+th1+a1+y1
...
c3a<-coord_sf(xlim = c(-116, -100), ylim = c(12, 26))

# bud point

# bl<-bud_fig1[bud_fig1$print_date == 10,] %>% na.omit()
# bl$label<-"Hurricane Bud"
# moved ecuase of new float
bl<-bud_fig1[bud_fig1$print_date == 9,] %>% na.omit()
...
m.map4_ws<-ggplot()+c1+c2+t1+t2+t3+p1a+p2a+p1b+ggtitle("Hurricanes Olaf & Bud with Argo Float Tracks")+theme(plot.title = element_text(size = title.size))+all_stations+all_stations_labled+c3a+l1+l2+th1+a1 + bud_label +olaf_label
...
setwd(wd)
setwd(fig)
pdf("Olaf_Bud_and_3_argo_floats.pdf", height=8.5 , width =7.5)
p2
dev.off()
```
